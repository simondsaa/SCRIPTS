				...
				If($Check_Connection -eq $true)
					{
						...
						$Script:Get_manufacturer = Invoke-Command -ComputerName $Computer -ScriptBlock $Get_Remote_Vendor -credential $Creds -ErrorVariable errmsg 2>$null	
						If($errmsg -ne $null)
							{
		...
		Function Set_Dell_BIOS_Settings
			{						
				...
				If(($Muliple) -or ($Computer -ne ""))
					{
						$Exported_CSV = "c:\windows\temp\Exported_BIOS_Settings.csv"																		
    ...
    Process
    {
       ...
       switch ($manufacturer)
       {
           'Dell'
           {
						...
						ForEach($Computer_Line in $Get_List_Content)
							{
								...
								If($Check_Connection -eq $true)
									{
										Invoke-Command -ComputerName $Computer_Line -ScriptBlock ${Function:Set_Dell_BIOS_Settings} -ArgumentList (,$CSV_Content), $MyPassword -credential $Creds																		
									}
								Else
									{
				...
				ElseIf(($Computer -ne ""))
					{						
						Invoke-Command -ComputerName $Computer -ScriptBlock ${Function:Set_Dell_BIOS_Settings} -ArgumentList (,$CSV_Content), $MyPassword -credential $Creds								
					}
				Else
					{
				...
				{		
						...
						{										
							...
							ForEach($Computer_Line in $Get_List_Content)
								{								
									...
									If($Check_Connection -eq $true)
										{
											Invoke-Command -ComputerName $Computer_Line -ScriptBlock ${Function:Set_HP_BIOS_Settings} -ArgumentList (,$CSV_Content), $MyPassword -credential $Creds		
										}
									Else
										{
											write-host "Connexion KO on $Computer_Line" -Foreground yellow  								
					...
					ElseIf(($Computer -ne ""))
						{						
							...
							Invoke-Command -ComputerName $Computer -ScriptBlock ${Function:Set_HP_BIOS_Settings} -ArgumentList (,$CSV_Content), $MyPassword -credential $Creds										
						}
					Else
						{						
           ...
           'Lenovo'
           {
						...
						ForEach($Computer_Line in $Get_List_Content)
							{
								...
								If($Check_Connection -eq $true)
									{
										Invoke-Command -ComputerName $Computer_Line -ScriptBlock ${Function:Set_Lenovo_BIOS_Settings} -ArgumentList (,$CSV_Content), $MyPassword -credential $Creds		
									}
								Else
									{
										write-host "Connexion KO on $Computer_Line" -Foreground yellow  								
				...
				ElseIf(($Computer -ne ""))
					{
						Invoke-Command -ComputerName $Computer -ScriptBlock ${Function:Set_Lenovo_BIOS_Settings} -ArgumentList (,$CSV_Content), $MyPassword -credential $Creds														
					}
				Else
					{
