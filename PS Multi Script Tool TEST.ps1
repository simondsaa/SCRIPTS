...
$Comp = Read-Host "User Name"
    ...
    ElseIf ($Comp -eq "Ballentine"){$Compname = "XLWUW-491S9F"}
...
$b.SaveAs("C:\temp\Contacts_User_Info.xls")
}
#===================================================================
Function CDROM
{
...
Function MultiCopy
{
Write-Host -ForegroundColor Blue -BackgroundColor White "This will put file in temp folder on remote PC" 
$Path = Read-Host "Path to PCs"
...
function delete-remotefile {
    PROCESS {
                Write-Host -ForegroundColor Blue -BackgroundColor White "NOTE: * will remove all files in Temp folder."
...
#add computers to computers variable to search for profiles on those computers 
    $Path = Read-Host "Path to PCs"
    [array] $Computers = Get-Content $Path
...
{
start-process "C:\Program Files\Internet Explorer\iexplore.exe" "https://www.youtube.com/watch?v=SxmwFvf6bUA/"
}
#===================================================================
Function StartPS1
{
...
function Call-ADTool_pff {
    ...
    $SearchSG = "CN=!601,OU=Tyndall AFB Security Groups,OU=Tyndall AFB,OU=AFCONUSEAST,OU=Bases,DC=AREA52,DC=AFNOAPPS,DC=USAF,DC=MIL"
    ...
    # --------------------------------------------------------------
    # Handle clicking the "AOC/AFNORTH Users" button.
    # --------------------------------------------------------------
	$btnPhones_Click={
		Initialize-Listview
		$SBPStatus.Text = "Retrieving AOC/AFNORTH personnel..."
		"Name" | %{Add-Column $_}
		Resize-Columns
		$Col0 = $lvMain.Columns[0].Text
        $Info = Get-ADGroup -Filter {Name -like '!601'} -SearchBase $SearchSG | Get-ADGroupMember | Select-Object Name | Sort-Object Name
        Start-Sleep -m 400
        $Info | %{
            $Item = New-Object System.Windows.Forms.ListViewItem($_.$Col0)
            foreach ($Col in ($lvMain.Columns | ?{$_.Index -ne 0})){$Field = $Col.Text;$Item.SubItems.Add([string]$_.$Field)}
...
Do
{
    ...
    Write-Host "11 - Enter-PSSession (testing)"
    Write-Host -ForegroundColor Yellow "12 - PCs per BLDG & RM"
    Write-Host -ForegroundColor Yellow "13 - PC & User"
    Write-Host "14 - CD-Rom"
    Write-Host -ForegroundColor Gray "15 - Create Folder/Directory"
    Write-Host -ForegroundColor Gray "16 - Copy File -" -NoNewline
    Write-Host " (Single PC)" -ForegroundColor Red
    Write-Host -ForegroundColor Gray "17 - Copy File -" -NoNewline
    Write-Host " (Multi PCs)" -ForegroundColor Red
    Write-Host -ForegroundColor Gray "18 - Delete File -" -NoNewline
    Write-Host " (Multi PCs)" -ForegroundColor Red
    Write-Host -ForegroundColor Gray "19 - Move File -" -NoNewline
    Write-Host " (Multi/Single)" -ForegroundColor Red
    Write-Host "20 - Delete Profiles"
    Write-Host -ForegroundColor Green "21 - Ping & NsLookup"
    Write-Host -ForegroundColor Green "22 - Ping Tool - Edit List 1st"
    Write-Host "23 - C$"
    Write-Host -ForegroundColor Yellow "24 - GPUpdate"
    Write-Host -ForegroundColor Yellow "25 - GPUpdate Multi"
    Write-Host -ForegroundColor Gray "26 - Stop-Process"
    Write-Host -ForegroundColor Gray "27 - Stop-Process Multi"
    Write-Host -ForegroundColor Gray "28 - Stop AFEMNS"
    Write-Host -ForegroundColor Gray "29 - Stop-Explorer"
    Write-Host "30 - PSRecon - Must run PS as Admin"
    Write-Host -ForegroundColor Green "31 - Run PS1 Remote"
    Write-Host -ForegroundColor Green "32 - Remote Install via PS1"
    Write-Host -ForegroundColor Green "33 - Remote Install via EXE"
    Write-Host "34 - SDC version"
    Write-Host -ForegroundColor Yellow "35 - AD Tool"
    Write-Host -ForegroundColor Yellow "36 - AD Group Contents Display"
    Write-Host -ForegroundColor Yellow "37 - AD User Info"
    Write-Host -ForegroundColor Green "38 - Create Scheduled Task -" -NoNewLine
    Write-Host " STILL TESTING" -ForegroundColor Red
    Write-Host -ForegroundColor Green "39 - Delete Scheduled Task"
    Write-Host -ForegroundColor Green "40 - Start Scheduled Task"
    Write-Host -ForegroundColor Green "41 - Show Scheduled Tasks"
    Write-Host "41 - Exit"
    Write-Host " "
