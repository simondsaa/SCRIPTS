...
$domain = "OU=Tyndall AFB,OU=AFCONUSEAST,OU=Bases,DC=AREA52,DC=AFNOAPPS,DC=USAF,DC=MIL"
$objDomain = [adsi]("LDAP://" + $domain)
$PCs = Read-Host "Path to PCs"
$Computers = Get-Content $PCs
$Path = "C:\Temp\Comp_Loc.txt"
#Write-Host "Results are located here:  C:\Temp\Comp_Loc.txt" -ForegroundColor Green
...
$Computers = Get-Content $Path
Wmic /node:@$Path computersystem get $Alias
}
#===================================================================
...
Do
{
    ...
    Write-Host -ForegroundColor Yellow "13 - PC: User, Bldg and Rm"
    Write-Host -ForegroundColor Magenta "14 - CD-Rom"
    ...
    Write-Host -ForegroundColor Gray "17 - Copy files to CST_Help - " -NoNewline
    Write-Host -ForegroundColor Blue -BackgroundColor Black "(Multi PCs)"
    Write-Host -ForegroundColor Gray "18 - Delete File"
    Write-Host -ForegroundColor Gray "19 - Move File - " -NoNewline
    Write-Host -ForegroundColor Blue -BackgroundColor Black "(Multi/Single)"
    Write-Host "20 - Delete Profile(s)"
    Write-Host "21 - Backup Profile(s)"
    Write-Host "22 - Copy Profile (for PXE)"
    Write-Host -ForegroundColor Green "23 - Ping & NsLookup"
    Write-Host -ForegroundColor Green "24 - Ping Tool"
    Write-Host "25 - C$"
    Write-Host -ForegroundColor Yellow "26 - GPUpdate"
    Write-Host -ForegroundColor Yellow "27 - GPUpdate Multi"
    Write-Host -ForegroundColor Gray "28 - Stop-Process"
    Write-Host -ForegroundColor Gray "29 - Stop-Process Multi"
    Write-Host -ForegroundColor Gray "30 - Stop AFEMNS"
    Write-Host -ForegroundColor Gray "31 - Stop-Explorer"
    Write-Host "32 - PSRecon - Must run PS as Admin"
    Write-Host -ForegroundColor Green "33 - Run PS1 Remote"
    Write-Host -ForegroundColor Green "34 - Remote Install via PS1"
    Write-Host -ForegroundColor Green "35 - Remote Install via EXE"
    Write-Host "36 - SDC version"
    Write-Host -ForegroundColor Yellow "37 - AD Tool"
    Write-Host -ForegroundColor Yellow "38 - AD Group Contents Display"
    Write-Host -ForegroundColor Yellow "39 - AD User Info - (Last Name)"
    Write-Host -ForegroundColor Yellow "40 - AD User Info - (EDI)"
    Write-Host -ForegroundColor Green "41 - Create Scheduled Task - " -NoNewLine
    Write-Host -ForegroundColor Red -BackgroundColor White "STILL TESTING"
    Write-Host -ForegroundColor Green "42 - Delete Scheduled Task"
    Write-Host -ForegroundColor Green "43 - Start Scheduled Task"
    Write-Host -ForegroundColor Green "44 - Show Scheduled Tasks"
    Write-Host -ForegroundColor Yellow "45 - Enable PS Remoting"
    Write-Host -ForegroundColor Yellow "46 - Enable WinRm - Run this 1st, then PS Remoting"
    Write-Host -ForegroundColor Yellow "47 - PS Exec Session - " -NoNewline
    Write-Host -ForegroundColor Red -BackgroundColor White "STILL TESTING"
    Write-Host -ForegroundColor Magenta "48 - Speak"
    Write-Host -ForegroundColor Magenta "49 - Speak Multi"
    Write-Host -ForegroundColor Gray "50 - Set Time"
    Write-Host "51 - Desktop Folders"
    Write-Host "52 - Copy Item to Users' Desktops"
    Write-Host "53 - Get PC Information"
    Write-Host "54 - Create Local Admin"
    Write-Host "55 - Start Service - " -NoNewline
...
schtasks.exe /CREATE /TN $TaskName /S $Comp /SC ONCE /sd $Date /st $Time /RL HIGHEST /RU SYSTEM /TR "'$EXE'" /F
    ...
    {
           Write-Host " "
           Write-Host "Be sure to enable Remote Registry service before you run this!" -foregroundColor Green
           Write-Host " "
     $PW = Write-Host "Press" -NoNewline 
           Write-Host " 1" -ForegroundColor Green -NoNewline
