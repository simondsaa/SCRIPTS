...
#Target List
#$Comps = Get-Content "ENTER PATH TO COMPUTER NAME LIST FILE"
...
$scriptblock = {
    ...
    TRY {
            ...
            IF (Test-Connection $args -Quiet -Count 1 -buffersize 16) {
                #ONLINE
                $ping = "Online"
                $create = schtasks.exe /CREATE /TN "Fix Vuln" /S $args /SC ONLOGON /RL HIGHEST /RU SYSTEM /TR "powershell.exe -noprofile -File 'PATH TO INSTALL PS1'" /F  
                Sleep -s 1
