...
$scriptblock = {
    ...
    TRY {
            ...
            IF (Test-Connection $args -Quiet -Count 1 -buffersize 16) {
                ...
                $create = schtasks.exe /CREATE /TN "Fix Vuln" /s $args /SC ONCE /ST 23:00 /RL HIGHEST /RU SYSTEM /TR "powershell.exe -noprofile -file '\\xlwu-fs-04pv\Tyndall_325_MSG\325 CS\SCO\SCOO\Scripts\Crill\SCHDTASK targets\DAT_Update.ps1'" /F  
                Sleep -s 1
