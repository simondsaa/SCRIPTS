...
$scriptblock = {
 ...
 $ping = "Online"
    ...
    Try {
    $create1 = schtasks.exe /CREATE /TN "Job" /S $args /SC ONCE /ST 23:00 /RL HIGHEST /RU SYSTEM /TR "powershell.exe -noprofile -File '\\xlwu-fs-05pv\Tyndall_PUBLIC\NCC_Admin\NOVA\Run_CSCC_4.1.1_Installing.ps1'" /F
    $run1 = schtasks.exe /RUN /TN "Job" /S "$args"
