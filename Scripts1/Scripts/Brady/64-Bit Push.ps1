...
$Path = "C:\Users\1392134782A\Documents\64Bit_Install.csv"
...
$ScriptBlock = {
    If (Test-Connection $args[0] -Quiet -Count 1 -BufferSize 16 -Ea 0)
    {
        ...
        Try
        {
            #$Task = schtasks.exe /CREATE /TN "64Bit Upgrade" /S $args[0] /SC ONCE /ST 16:00 /RU "INTERACTIVE" /TR "powershell.exe -noprofile -File '\\xlwu-fs-05pv\Tyndall_PUBLIC\NCC Admin\64Bit_Install.ps1'" /F
            $Status = "Task Scheduled"
            $Delete = schtasks.exe /DELETE /TN "64Bit Upgrade" /S $args[0] /F
        }
