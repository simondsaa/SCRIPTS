...
$Path = "\\XLWUW3-DKPVV1\C$\Users\1392134782A\Documents\SystemsBios.csv"
...
$ScriptBlock = {
    If (Test-Connection $args[0] -Quiet -Count 1 -BufferSize 16 -Ea 0)
    {
        ...
        Try
        {
            $BIOS = (Get-WmiObject Win32_BIOS -cn $args[0] -ErrorAction SilentlyContinue).Name
        }
...
$Outcome | Select System, Ping, BIOS, Organization, Building, Room -ExcludeProperty RunspaceId | Export-Csv $Path -Force
Import-Csv $Path | OGV
