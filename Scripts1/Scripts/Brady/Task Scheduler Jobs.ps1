...
$Path = "C:\Temp\Task Schedule $Date.csv"

# Max number of Jobs to run at one time (60 was found to be the sweet spot)
$MaxThreads = 60

# Number of seconds before the script cancels and hung up jobs (for a small number of computers it can be reduced to ~30)
$TimeOut = 150
...
$ScriptBlock = {
    ...
    {    
        If (Test-Connection $args[0] -Quiet -Count 1 -BufferSize 16 -Ea 0)
        {
            ...
            $Task = schtasks.exe /CREATE /TN "Acrobat" /S $args[0] /SC ONLOGON /RU "INTERACTIVE" /TR "powershell.exe -WindowStyle Hidden -noprofile -File '\\xlwu-fs-05pv\Tyndall_PUBLIC\NCC Admin\Flash_Install.ps1'" /F
            Sleep -s 1
            $Run = schtasks.exe /RUN /TN "Acrobat" /S $args[0] 
            Sleep -s 1
            $Delete = schtasks.exe /DELETE /TN "Acrobat" /S $args[0] /F
            $Success = "Successful"
        }
        Else
        {
