...
$nl
Write-host "Input path you'd like to pull results for."
$Ans = Read-host "Path:"
...
$DirTOP = Get-Childitem "$Ans" -Directory |Sort-object | Select Fullname
} 
ELSE { # Universal PSVersion scan
$DirTOP = Get-Childitem "$AnsC" | where {$_.PsIsContainer} | Sort-object | Select-Object FullName
}
...
$ErrorActionPreference = 'SilentlyContinue'
                ...
                $task = $args | Get-Childitem -directory -recurse | Get-ACL | Select-Object @{Name="Path";Expression={$_.PSPath.Substring($_.PSPath.IndexOf(“:”)+2) }},Owner,AccessToString
...
"Your outcome is stored in the Outcome variable, as well as on your desktop named FileScan_DATE"
$Date = Get-Date -UFormat "%d-%b-%g %H%M" 

$outcome | Export-Csv "$env:USERPROFILE\desktop\FileScan_$date.csv"
"Opening Outgrid View"
$outcome | ogv
