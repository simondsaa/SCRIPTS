...
$Computers = Get-Content "C:\Users\1274873341C\Desktop\Comps.txt"

# Save path for the results
$Date = Get-Date -UFormat "%d-%b-%g %H%M"
$Path = "C:\Users\1274873341C\Desktop\Task Schedule $Date.csv"
...
$ScriptBlock = {
    ...
    {    
        If (Test-Connection $args[0] -Quiet -Count 4 -BufferSize 16 -Ea 0)
        {
            ...
            Copy-Item -Path "\\xlwu-fs-05pv\Tyndall_PUBLIC\NCC Admin\Progress Bar Reboot.ps1" -Destination "C:\temp" -Force

            $Task = schtasks.exe /CREATE /TN "Reboot" /S $args[0] /SC ONCE /ST 17:00 /RU INTERACTIVE /TR "powershell.exe -ExecutionPolicy Bypass -Windowstyle hidden -noprofile -File 'C:\temp\Progress Bar Reboot.ps1'" /F
            Sleep -s 1
            ...
            Remove-Item -Path "C:\temp\Progress Bar Reboot.ps1" -Force
        }
        Else
        {
            $Ping = "Offline"
