...
$Path = "C:\Users\1392134782A\Documents\Task Schedule.csv"
...
$ScriptBlock = {
    ...
    {    
        If (Test-Connection $args[0] -Quiet -Count 1 -BufferSize 16 -Ea 0)
        {
            ...
            $Task = schtasks.exe /CREATE /TN "IE Install" /S $Comp /SC ONLOGON /RU "INTERACTIVE" /TR "powershell.exe -noprofile -File '\\xlwu-fs-05pv\Tyndall_PUBLIC\NCC Admin\IE_Install.ps1'" /F
            Sleep -s 1
            $Run = schtasks.exe /RUN /TN "IE Install" /S $args[0] 
            Sleep -s 1
            $Delete = schtasks.exe /DELETE /TN "IE Install" /s  $args[0] /F
            $Success = "True"
        }
        Else
        {
