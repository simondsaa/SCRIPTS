...
#Target List
$Comps = get-content "\\xlwu-fs-05pv\tyndall_public\Tester.txt"

$program = "LastLoggedOnSAMUser"
            ...
            IF (Test-Connection $args[0] -Quiet -Count 1 -buffersize 16) {
                ...
                $success = "True"
                    ...
                    If ($OSInfo.OSArchitecture -eq "64-bit"){
                            $RegPath = "Software\\Microsoft\\windows\\currentVersion\\Authentication"
                            }
                    ElseIf ($OSInfo.OSArchitecture -eq "32-bit"){
                            ...
                            Try {        
                                ...
                                $Program_Name = $thisSubKey.GetValue("LastLoggedOnSAMUser")
                        ...
                        Try {
                          ...
                          $FreeC = (get-wmiobject win32_volume| Measure-Object -Property FreeSpace -Sum).Sum/1gb
                          }
                        Catch { }
