...
# This script is designed to force remote workstations to run an SCCM package to install/repair a given application and reboot if you wish
#
# Things you need:
#    1. Network share where Domain Computers has modify access in the security tab
#    2. Text file with a list of computer names
#    3. Save the "Progress Bar Reboot.ps1" in the network path above
# 
# I recommend running this in a controlled environemt first to test funcionality and observe results before deploying

# CHANGES
# -------

# Network path for computers to reach out to, example - \\Server-1\Admin_Share (no spaces please, and no trailing \)
$NPath = "\\xlwu-fs-05pv\Tyndall_PUBLIC\NCC_Admin\Remediation_Testing"
...
# Check to make sure the Network Path provided is valid
If (!(Test-Path "$NPath"))
{
	Write-Host "Invalid network path, please enter a valid path and try again"
	Exit
}
...
$dialog.InitialDirectory = "C:\Users\1274873341C\Desktop\Desktop\PS_Scripts\Remediation_Tracking"
...
" + "`$SCCM.ExecuteProgram(`"$ID`",`"$PackageID`",`$true)" | Out-File -FilePath "$NPath\SCCM_Installer_$ID.ps1" -Force

$Script = "SCCM_Installer_$ID.ps1"

# Prompts for a reboot if needed/wanted
Write-Host
$Prompt = New-Object -ComObject Wscript.Shell
$Reboot = $Prompt.Popup("Will this installation require the remote system to be rebooted?",0,"Reboot Required",84)
...
$ScriptBlock = {
    ...
    {    
        If (Test-Connection $args[0] -Quiet -Count 4 -BufferSize 16 -Ea 0)
        {
            ...
            Copy-Item -Path "$($args[3])\$($args[1])" -Destination "\\$($args[0])\c$\temp" -Force
            ...
            If ($args[2] -eq 6)
            {
                ...
                Copy-Item -Path "$($args[3])\Progress Bar Reboot.ps1" -Destination "\\$($args[0])\c$\temp" -Force
...
ForEach ($Computer in $Computers)
{
    ...
    Start-Job -Name $Computer -ScriptBlock $ScriptBlock -ArgumentList $Computer, $Script, $Reboot, $NPath | Out-Null
