...
$Computers = Get-Content "C:\Users\1274873341C\Desktop\Desktop\PS_Scripts\_Java\Java_Patch.txt"

# Save path for the results
$Path = "C:\Users\1274873341C\Desktop\Desktop\PS_Scripts\_Java\Java_Results.csv"
...
$ScriptBlock = {
    ...
    {    
        If (Test-Connection $args[0] -Quiet -Count 1 -BufferSize 16 -Ea 0)
        {
            ...
            $Task = schtasks.exe /CREATE /TN "Java Update" /S $args[0] /SC ONLOGON /RL HIGHEST /RU SYSTEM /TR "powershell.exe -ExecutionPolicy Unrestricted -WindowStyle Hidden -noprofile -File C:\TEMP\Java\Deploy-Application.ps1" /F
            Sleep -s 1
            $Run = schtasks.exe /RUN /TN "Java Update" /S $args[0] 
            Sleep -s 1
            #$Delete = schtasks.exe /DELETE /TN "Java Update" /s $args[0] /F
            $Success = "True"
        }
        Else
        {
...
ForEach ($Computer in $Computers)
{
    ...
    $sourcefile = "\\xlwu-fs-05pv\Tyndall_PUBLIC\Applications\Java\Java_8.111\*"
    $destinationFolder = "\\$computer\C$\TEMP\Java"
