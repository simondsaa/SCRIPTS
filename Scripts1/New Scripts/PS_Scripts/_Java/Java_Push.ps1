...
$Path = "\\xlwu-fs-05pv\Tyndall_PUBLIC\Patching\Java\JavaPush.csv"
...
$ScriptBlock = {
    If (Test-Connection $args[0] -Quiet -Count 1 -BufferSize 16 -Ea 0)
    {
        ...
        Try
        {
            $Task = schtasks.exe /CREATE /TN "Java" /S $args[0] /SC ONLOGON /RU "INTERACTIVE" /TR "powershell.exe -noprofile -File '\\xlwu-fs-05pv\Tyndall_PUBLIC\Patching\Java\Java_Install.ps1'" /F  
