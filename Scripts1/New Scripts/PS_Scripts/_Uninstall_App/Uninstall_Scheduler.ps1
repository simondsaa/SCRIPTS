...
# MODIFICATIONS
# -------------
# 21 Mar 16 L Stringer: Modifying script to allow modification to uninstall specified scripts from remote machines
...
$Computers = Get-Content "\\xlwu-fs-05pv\Tyndall_PUBLIC\Patching\Uninstall_App\Uninstall_Targets.txt"

# Save path for the results
$Path = "\\xlwu-fs-05pv\Tyndall_PUBLIC\Patching\Uninstall_App\Task Schedule.csv"
...
$ScriptBlock = {
    ...
    {    
        If (Test-Connection $args[0] -Quiet -Count 1 -BufferSize 16 -Ea 0)
        {
            ...
            $Task = schtasks.exe /CREATE /TN "Uninstall App" /S $Comp /SC ONLOGON /RU "INTERACTIVE" /TR "powershell.exe -noprofile -File '\\xlwu-fs-05pv\Tyndall_PUBLIC\Patching\Uninstall_App\Uninstall_Task.ps1'" /F
            Sleep -s 1
            $Run = schtasks.exe /RUN /TN "Uninstall App" /S $args[0] 
            Sleep -s 1
            $Delete = schtasks.exe /DELETE /TN "Uninstall App" /s  $args[0] /F
            $Success = "True"
        }
        Else
        {
