...
#Target List
$Comps = Get-Content "C:\Users\1392134782A\Desktop\Computers.txt"
...
$scriptblock = {
    ...
    TRY {
            ...
            IF (Test-Connection $args -Quiet -Count 1 -buffersize 16) {
                #ONLINE
                $ping = "Online"
                $Task = schtasks.exe /CREATE /TN "Delete Old Profiles" /S $args /SC WEEKLY /D SAT /ST 23:59 /RL HIGHEST /RU SYSTEM /TR "powershell.exe -ExecutionPolicy Unrestricted -WindowStyle Hidden -noprofile -File '\\xlwu-fs-05pv\Tyndall_PUBLIC\NCC Admin\Delete Old Profiles.ps1'" /F
                #Sleep -s 1
